...
const subjects = useMemo(() => semesterList.flatMap((s) => s.subjects) || [], [semesterList]);
  ...
  const handleDeptChange = React.useCallback((selectedDept: string) => {
    console.log(selectedDept);
    
    setSelectedDept(selectedDept)  // Update the department
    setSelectedProgram("")         // Reset program state when department changes
    setSelectedSubject("")         // Reset subject state when department changes
  }, [])
  ...
  return (
    <div className="bg-[#09090b] min-h-screen w-full p-5">
   
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 justify-items-center mb-8">

       <div className="text-white">
         <ComboboxDemo onChange={handleDeptChange} items={departments}  value="Search Departments"/>
       </div>
        <div className="text-white">
         <ComboboxDemo onChange={handleProgramChange} items={programs}  value="Search Program"/>
       </div>
